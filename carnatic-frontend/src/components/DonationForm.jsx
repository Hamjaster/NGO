    ...
    if (!donationInfo.name) {
      ...
      try {
        const { data } = await axios.post(
          "https://kyc-api.aadhaarkyc.io/api/v1/pan/pan-comprehensive",
          body,
          {
            headers: headers,
          }
    ...
    <div className="text-[#474848] h-screen overflow-y-scroll font-roboto mx-auto w-full">
      <section className="mx-auto w-full md:w-2/3 space-y-10 py-4  pb-8 bg-white shadow-lg px-4 sm:px-10 text-[#474848]">
        ...
        <div class="relative z-0 w-full  group">
          ...
          <label
            for="name"
            class="peer-focus:font-medium absolute text-lg text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 z-10 origin-[0] peer-focus:left-0 peer-focus:text-[#fe0248]  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"
          >
            Name (As in PAN Card)
          </label>
        </div>
        ...
        <div class="relative z-0 w-full  group flex flex-row items-center justify-between gap-3">
          <div className={`${member === "guest" ? "w-full" : "w-full"} '`}>
            <input
              disabled={
                member !== "guest" || (member === "guest" && panValidated)
              }
              value={donationInfo.PAN}
              onChange={(e) => updateInfo(e)}
              type="text"
              name="PAN"
              id="pan"
          ...
          <button
            ...
            className={`${
              member === "guest" ? "" : "hidden"
            } w-44    bg-[#4dd7fe] text-lg h-[3.1rem]  rounded-md disabled:bg-gray-400 hover:bg-[#00c8ff] text-white`}
          >
            {panverifyloading ? (
          ...
          <div class="relative z-0 w-full group">
            <div className="flex flex-row gap-5">
              ...
              <button
                ...
                className={`${
                  member === "guest" ? "" : "hidden"
                } bg-[#4dd7fe] text-lg py-1 w-32 rounded-md hover:bg-[#00c8ff] text-white`}
              >
                {otpvalidateLoading ? (
                  <div className="animate-spin mx-auto text-2xl max-w-min">
                    <CgSpinner />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div class="relative z-0 w-full group">
            <input
              ...
              class={`"block py-2.5 px-2 w-full text-lg text-gray-900 rounded-lg bg-transparent border-2 
                            ${
                              isValidEmail
                                ? "border-gray-300"
                                : "border-red-500"
        ...
        <div
          ...
          className={`relative z-0 select-none text-center w-full`}
        >
          <div
            className={`flex flex-row items-center text-lg justify-center py-2 cursor-pointer space-x-7 hover:bg-[#fe1648] hover:text-white border border-[#fe1648]`}
          >
            {project ? (
              <>Donating to : {project} </>
            ) : (
          ...
          <div
            ...
            class={`${
              open ? "" : "hidden"
            } top-full left-0 right-0 z-10 absolute select-none  w-full divide-y divide-gray-100 rounded-lg `}
          >
            <ul
          ...
          <div class="flex items-center space-x-8">
            <div className="parent flex items-center">
              <input
                ...
                id="check2"
                type="checkbox"
                class="w-4 h-4 text-[#D60036] bg-gray-100 border-gray-300 rounded "
              />
              <label
                for="check2"
        ...
        <button
          ...
          onClick={() => {
            member === "guest" ? createUser() : navigate("/amount");
          }}
        ...
        <div id="recaptcha h-44 w-44"></div>
      </section>
    </div>
  );
}
